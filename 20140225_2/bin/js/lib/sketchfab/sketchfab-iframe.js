...
define("vendors/almond/almond", function(){});
...
define('api/sketchfab-iframe',[ "vendors/q/q.min"], function( Q ) {
    ...
    var API = function(iframe) {
        ...
        if ( window.location.href.search("sketchfab") === -1 ) {
            ...
            var parts = window.location.hostname.split(/\./g);
            this._domain = "//" + ( parts[ parts.length - 1 ] === 'me' ? parts[ parts.length - 2 ] + '.com' : window.location.hostname );
        }
    ...
    var handleEvent = function(data, clear) {
        ...
        } else if ( this._events[data.status] ){
    ...
    API.prototype = {
        ...
        autospin: function(spin) {
            console.log('autospin');
            if (this._events.autospin !== undefined) {
                return this._events.autospin.promise;
            }

            this._events.autospin = Q.defer();
            this._iframe.contentWindow.postMessage({ command: "autospin",
                                                     spin: spin
                                                   }, '*');
 ...
 */
define('lib/sketchfab-iframe',['require','api/sketchfab-iframe'],function (require) {
    ...
    return require( 'lib/sketchfab-iframe' );
} ) );
